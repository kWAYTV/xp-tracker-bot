...
class AddUser(commands.Cog):
    ...
    async def add_track_user(self, interaction: discord.Interaction, id: str, hidden: bool = True):
        ...
        if self.database.get_user_by_steam_id(steamid64) is not None:
            await added_message.edit(content=f"The id `{id}` is already being tracked.")
        ...
        try:
            ...
            await added_message.edit(content=f"Couldn't get level and XP for `{id}`, couldn't add him to the tracker database. Error: {e}")
